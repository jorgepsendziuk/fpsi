-- Alinha a view ao cálculo do diagnóstico:
-- 1) INCC: score do controle = (baseIndex/2) * (1 + incc_nivel/5), onde incc_nivel vem de
--    programa_controle.nivel (id 1→0, 2→1, 3→2, 4→3, 5→4, 6→5). Igual a useMaturityCache/ControleContainer.
-- 2) Denominador: total_medidas inclui todas as medidas do controle exceto "Não se aplica" (resposta 6),
--    inclusive não respondidas (como no app), para que baseIndex = soma_pesos/total_medidas bata com o diagnóstico.
...
-- Total de medidas por controle: todas exceto "Não se aplica" (6), incluindo não respondidas (igual ao app)
medida_count AS (
  SELECT
    programa_id,
    controle_id,
...
-- INCC: programa_controle.nivel id 1-6 → 0-5; score = (baseIndex/2)*(1+incc_nivel/5)
score_controle AS (
  SELECT
    cs.programa_id,
    cs.controle_id,
  ...
  JOIN medida_count mc
    ON mc.programa_id IS NOT DISTINCT FROM cs.programa_id
   AND mc.controle_id IS NOT DISTINCT FROM cs.controle_id
   AND mc.diagnostico_id IS NOT DISTINCT FROM cs.diagnostico_id
   AND mc.controle_numero IS NOT DISTINCT FROM cs.controle_numero
  LEFT JOIN public.programa_controle pc
    ON pc.programa IS NOT DISTINCT FROM cs.programa_id
   AND pc.controle IS NOT DISTINCT FROM cs.controle_id
),
imc0 AS (
