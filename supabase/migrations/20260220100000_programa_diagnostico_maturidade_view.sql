-- View: índices de maturidade por programa e diagnóstico (Estrutura, Segurança, Privacidade)
-- Espelha a lógica do useMaturityCache: pesos por resposta, INCC, iMC0, fórmulas iSeg/iPriv.

CREATE OR REPLACE VIEW public.programa_diagnostico_maturidade AS
WITH
-- Resposta como inteiro (programa_medida.resposta é text)
pm_resposta AS (
  SELECT
    ...
    NULLIF(TRIM(pm.resposta), '')::integer AS resposta_num
...
-- Peso por resposta: diag 1 (Sim/Não), diag 2/3 (escala 1-5). Excluir resposta 6 (Não se aplica).
medida_peso AS (
  SELECT
    programa_id,
    controle_id,
...
-- Contagem de medidas por controle (excl. não se aplica) para denominador
medida_count AS (
  SELECT
    programa_id,
    controle_id,
...
-- Soma de pesos por (programa, controle)
controle_soma AS (
  SELECT
    programa_id,
    controle_id,
...
-- Score por controle: (baseIndex/2) * (1 + incc_nivel/5), nivel 1..6 -> 0..5
score_controle AS (
  SELECT
    cs.programa_id,
    cs.controle_id,
...
-- iMC0 = score do controle com numero = 0 por (programa, diagnostico)
imc0 AS (
  SELECT programa_id, diagnostico_id, score AS imc0_score
  FROM score_controle
  WHERE controle_numero = 0
),
-- Demais controles (numero <> 0) agregados por (programa, diagnostico)
demais AS (
  SELECT
    programa_id,
    diagnostico_id,
...
-- Score final por diagnóstico: diag 1 = iMC0; diag 2/3 = ((iMC0*4)+soma)/(4+qtd)
score_diag AS (
  SELECT
...
-- Uma linha por (programa, diagnóstico): base para LEFT JOIN do cálculo
base AS (
...
'Índices de maturidade por programa e diagnóstico (1=Estrutura, 2=Segurança, 3=Privacidade). Score 0-1, label Inicial/Básico/Intermediário/Em Aprimoramento/Aprimorado.';
