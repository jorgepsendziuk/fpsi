-- ============================================
-- Seed fictício para o programa de DEMONSTRAÇÃO FPSI:
-- mapeamento de dados, ROPA (registro + operações), RIPD, responsáveis,
-- respostas de medidas / níveis INCC em controles, pedidos de titulares (art. 18).
-- Idempotente: remove apenas linhas com prefixo "Demo FPSI —" / protocolos PT-DEMO-*.
-- Resolve programa: slug demonstracao ou demo, senão id 1.
-- ============================================
...
BEGIN
  ...
  IF pid IS NULL THEN
    RAISE NOTICE 'seed_demo_completo: nenhum programa demo/demonstracao ou id 1; nada feito.';
    RETURN;
  END IF;

  RAISE NOTICE 'seed_demo_completo: programa_id=%', pid;
  ...
  RAISE NOTICE 'seed_demo_completo: concluído para programa_id=%', pid;
END $$;
