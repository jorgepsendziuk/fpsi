...
interface PerfilContentProps {
  /** Quando true, usa layout compacto para dialog (sem título grande) */
  compact?: boolean;
}

export function PerfilContent({ compact = false }: PerfilContentProps) {
  ...
  const handleAvatarUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok && data.avatar_url) {
        setProfile((p) => (p ? { ...p, avatar_url: data.avatar_url } : { user_id: user!.id, avatar_url: data.avatar_url }));
      }
    } finally {
    ...
    <Stack spacing={3}>
      ...
      >
        <Box sx={{ display: "flex", alignItems: "center", gap: 2, mb: 2 }}>
          <Box component="label" sx={{ position: "relative", cursor: avatarUploading ? "wait" : "pointer" }}>
            <Avatar
