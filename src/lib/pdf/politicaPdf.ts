...
export type PoliticaPdfSection = {
  id: number;
  secao: string;
  titulo?: string;
  texto?: string;
...
export async function generatePoliticaPdfBytes(input: {
  sections: PoliticaPdfSection[];
  politicaNome?: string;
  nomeFantasia?: string;
  programa?: Record<string, unknown>;
}): Promise<{ bytes: Uint8Array; filename: string }> {
  const { sections, politicaNome, nomeFantasia, programa } = input;
  ...
  const filename = `${String(docTitle).replace(/[/\\?%*:|"<>]/g, "-").slice(0, 80)}.pdf`;
